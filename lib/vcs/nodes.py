...
from lib.vcs.exceptions import NodeError
from lib.vcs.exceptions import RemovedFileNodeError
...
class FileNode(Node):
    ...
    def content(self):
        ...
        if bool(content and b'\0' in content):
            return content
        return content
    ...
    def __json__(self):
        result = dict(
            kind=self._kind,
            changeset=self.changeset.__json__(),
            name=self.name,
            path=self.path,
            size=self.size,
        )
        if hasattr(self, '_blob'):
            result.update({"data": self._blob.data.decode("utf-8")})
        return result
